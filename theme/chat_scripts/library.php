...
library index
	...
	DB Retrieval
		...
		get_resolution($conn,$start,$end,$dname)
		get_name($conn,$cid)
		get_style($conn,$score,$metric,$type)
	...
	function get_surveys($conn,$to,$from,$id) {
		...
		if(count($team) == 1) {
			...
			foreach($team AS $member) {
		        ...
		        $PDO = $conn->prepare('SELECT tid,tid2 FROM roster WHERE cid = :cid');
		        $PDO->bindParam(':cid', $member, PDO::PARAM_STR);
		        $PDO->execute();
				...
				if(count($team) != $x) {
					$sql .= "rep_id = '".$tid1."' OR rep_id = '".$tid2."' OR ";
				} else {
					$sql .= "rep_id = '".$tid1."' OR rep_id = '".$tid2."'";
				}
				$x++;
			}
		}
		...
		try {
	        ...
	        $PDO = $conn->prepare('SELECT rep_id,completion_date,metric_1,metric_2,metric_3 FROM survey_data WHERE ('.$sql.')');
...
//////////////////////////////////////////////
    ...
    try {
        ...
        $PDO = $conn->prepare('SELECT driver_1 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) UNION SELECT driver_2 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) UNION SELECT driver_3 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) UNION SELECT driver_4 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) UNION SELECT driver_5 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) UNION SELECT driver_6 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) UNION SELECT driver_7 FROM survey_data WHERE (completion_date BETWEEN :start AND :end)');
        $PDO->bindParam(':start', $start, PDO::PARAM_STR);
        $PDO->bindParam(':end', $end, PDO::PARAM_STR);
        $PDO->execute();
...
//////////////////////////////////////////////
	...
	function get_resolution($conn,$start,$end,$dname) {
    
    try {
        
        $sql2 = '(driver_1 = "'.$dname.'" OR driver_2 = "'.$dname.'" OR driver_3 = "'.$dname.'" OR driver_4 = "'.$dname.'" OR driver_5 = "'.$dname.'" OR driver_6 = "'.$dname.'" OR driver_7 = "'.$dname.'")';
        $PDO = $conn->prepare('SELECT res_1 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) AND '.$sql2.' UNION SELECT res_2 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) AND '.$sql2.' UNION SELECT res_3 FROM survey_data WHERE (completion_date BETWEEN :start AND :end) AND '.$sql2.'');
        $PDO->bindParam(':start', $start, PDO::PARAM_STR);
        $PDO->bindParam(':end', $end, PDO::PARAM_STR);
        //$PDO->bindParam(':l1', $l1, PDO::PARAM_STR);
        $PDO->execute();
...
//////////////////////////////////////////////
	...
	function create_table($conn,$pstart,$pend,$cstart,$cend,$l1,$team,$cid,$view,$scount) {
		...
		$table .= "<table class='table table-striped table-bordered2 table-hover' id='sample_3'>
								<thead>
									<tr>
								...
								foreach ($list AS $value) {
									...
									if($view == '0') {
										$line = create_table_row($conn,$pstart,$pend,$cstart,$cend,$value['driver_1'],'driver',$cid,$team,$scount);
										$item = $value['driver_1'];
									} else { 
	...
	function create_table_row($conn,$pstart,$pend,$cstart,$cend,$dname,$driver,$cid,$team,$scount) {
		...
		if(count($team) == 1) {
			...
			try {
		        ...
		        $PDO = $conn->prepare('SELECT tid,tid2 FROM roster WHERE cid = :cid');
		        $PDO->bindParam(':cid', $dname, PDO::PARAM_STR);
		        $PDO->execute();
		...
		if($driver == 'team') {
		...
		} elseif ($driver == 'driver') {
			
			try {

				$sql2 = '(driver_1 = "'.$dname.'" OR driver_2 = "'.$dname.'" OR driver_3 = "'.$dname.'" OR driver_4 = "'.$dname.'" OR driver_5 = "'.$dname.'" OR driver_6 = "'.$dname.'" OR driver_7 = "'.$dname.'")';
	        
		        $PDO = $conn->prepare('SELECT metric_1,metric_2,metric_3 FROM survey_data WHERE ('.$sql.') AND '.$sql2.' AND completion_date BETWEEN :start AND :end');
		        $PDO->bindParam(':start', $pstart, PDO::PARAM_STR);
		        $PDO->bindParam(':end', $pend, PDO::PARAM_STR);
		        $PDO->execute();
		    ...
		    try {
		        ...
		        $PDO2 = $conn->prepare('SELECT metric_1,metric_2,metric_3 FROM survey_data WHERE ('.$sql.') AND '.$sql2.' AND completion_date BETWEEN :start AND :end');
		        $PDO2->bindParam(':start', $cstart, PDO::PARAM_STR);
		        $PDO2->bindParam(':end', $cend, PDO::PARAM_STR);
		        $PDO2->execute();
		...
		} elseif ($driver == 'level_3') {

			

		    try {

				//$sql2 = '(driver_1 = "'.$driver.'" OR driver_2 = "'.$driver.'" OR driver_3 = "'.$driver.'" OR driver_4 = "'.$driver.'" OR driver_5 = "'.$driver.'" OR driver_6 = "'.$driver.'" OR driver_7 = "'.$driver.'")';
	        
		        $PDO = $conn->prepare('SELECT metric_1,metric_2,metric_3 FROM survey_data WHERE ('.$sql.') AND '.$driver.' = "'.$dname.'" AND resolution = :resolution AND completion_date BETWEEN :start AND :end');
		        $PDO->bindParam(':start', $pstart, PDO::PARAM_STR);
		        $PDO->bindParam(':end', $pend, PDO::PARAM_STR);
		        $PDO->bindParam(':resolution', $dname, PDO::PARAM_STR);
		        $PDO->execute();
		    ...
		    try {
		        ...
		        $PDO2->bindParam(':start', $cstart, PDO::PARAM_STR);
		        $PDO2->bindParam(':end', $cend, PDO::PARAM_STR);
		        $PDO2->bindParam(':resolution', $dname, PDO::PARAM_STR);
		        $PDO2->execute();
		    ...
		    try {
				...
				$sql2 = '(driver_1 = "'.$driver.'" OR driver_2 = "'.$driver.'" OR driver_3 = "'.$driver.'" OR driver_4 = "'.$driver.'" OR driver_5 = "'.$driver.'" OR driver_6 = "'.$driver.'" OR driver_7 = "'.$driver.'")';
	        	$sql3 = '(res_1 = "'.$dname.'" OR res_2 = "'.$dname.'" OR res_3 = "'.$dname.'" OR res_4 = "'.$dname.'")';


		        $PDO = $conn->prepare('SELECT metric_1,metric_2,metric_3 FROM survey_data WHERE ('.$sql.') AND '.$sql2.' AND '.$sql3.' AND completion_date BETWEEN :start AND :end');
		        $PDO->bindParam(':start', $pstart, PDO::PARAM_STR);
		        $PDO->bindParam(':end', $pend, PDO::PARAM_STR);
		        $PDO->execute();
		    ...
		    try {
		        ...
		        $PDO2 = $conn->prepare('SELECT metric_1,metric_2,metric_3 FROM survey_data WHERE ('.$sql.') AND '.$sql2.' AND '.$sql3.' AND completion_date BETWEEN :start AND :end');
		        $PDO2->bindParam(':start', $cstart, PDO::PARAM_STR);
		        $PDO2->bindParam(':end', $cend, PDO::PARAM_STR);
		        $PDO2->execute();
